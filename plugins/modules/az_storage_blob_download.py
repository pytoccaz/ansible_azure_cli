...
DOCUMENTATION = '''
---
module: az_storage_blob_download
author: "Olivier Bernard (@pytoccaz)"
short_description: Downloads a blob from an azure storage container
description:
...
def az_storage_download(module):

    az_bin = module.get_bin_path("az", required=True)

    command = [az_bin, "storage", "blob", "download",
    ...
    if module.params["overwrite"] is False:
        command.append("--overwrite=false")
...
def main():
    ...
    rc, out, err = az_storage_download(module)

    if rc != 0:
        module.fail_json(msg="Error running az storage blob download. {0}".format(err),
